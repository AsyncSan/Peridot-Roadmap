...
const App: React.FC = () => {
  ...
  // Check auth on mount
  useEffect(() => {
    const storedAuth = localStorage.getItem('peridot_auth');
    if (storedAuth === 'true') {
  ...
  const handleLoginSuccess = () => {
      localStorage.setItem('peridot_auth', 'true');
      setIsAuthenticated(true);
      // Trigger welcome sound or intro here if desired
      handleSpeak("Access Granted. Welcome to the Roadmap.");
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-100 font-sans relative overflow-hidden">
      {/* Background Blobs - Visible behind login gate too */}
      ...
      {!isAuthenticated && (
          <LoginGate onLogin={handleLoginSuccess} />
      )}
